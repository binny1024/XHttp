...
import static com.binny.sdk.common.CommonMethod.removeAll;
import static com.binny.sdk.common.ErrorCode.CODE_NO_AD_LIST_NET;
...
public class AdCompare implements IAdCompare, OnDownloadCallback {
    ...
    public void checkAdListForUpdate(Map<String, AdListBean.DataBean.ListBean> listBeanMapLocal, AdListBean adListBeanNet) {
        ...
        if (listBeanMapLocal != null) {
            ...
            //保存数据，同时缓存图片资源
            UtilCache.getAdInfoCacheHelper().putSerializableBean(Configuration.AD_LIST_CACHE, adListBeanNet);
            //缓存图片
        ...
        * */
        for (final Map.Entry<String, AdListBean.DataBean.ListBean> entry : listMapLocal.entrySet()) {
            ...
            if (!adListMapNet.containsKey(idOld)) {//第一步：移除缓存数据
                ...
                if (imgUrlNew.equals(imgUrlOld) && UtilCache.getAdBitmapCacheHelper().fileCacheExist(idOld)) {
         ...
         * 缓存最新的广告列表 和 广告实体数据
         * */
        UtilCache.getAdInfoCacheHelper().putSerializableBean(Configuration.AD_LIST_CACHE, adListNet);
    }
    ...
    public void onSuccess(String adId) {
        JJLogger.logInfo("update", "--------------------------------------------------广告 id = " + adId + "的广告，" + "保存成功 ？" + UtilCache.getAdBitmapCacheHelper().fileCacheExist(adId));
    }
